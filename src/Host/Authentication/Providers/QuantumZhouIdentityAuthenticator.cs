...
/// <summary>
/// Speaks QuantumZhou.Identity's proprietary token contract:
...
/// path, the header names, or the envelope shape, so a change on the Identity side —
/// including the HMAC-signed gateway protocol under discussion in PD-004 — lands in this
/// one file.
/// </summary>
public sealed class QuantumZhouIdentityAuthenticator : IAdminCredentialAuthenticator
{
    private static readonly JsonSerializerOptions SerializerOptions =
        new(JsonSerializerDefaults.Web);

    private readonly HttpClient _httpClient;
    private readonly QuantumZhouProviderOptions _options;
    private readonly ILogger<QuantumZhouIdentityAuthenticator> _logger;

    public QuantumZhouIdentityAuthenticator(
        IHttpClientFactory httpClientFactory,
        IOptions<QuantumZhouProviderOptions> options,
        ILogger<QuantumZhouIdentityAuthenticator> logger)
    {
